...
def shortest_path(maps, start, goal):
    ...
    return minimum_cost(maps, graph, start, goal)


def calculate_distance(maps, a, b):
    point_a = maps.intersections.get(a)
    x_a = point_a[0]
    y_a = point_a[1]
    point_b = maps.intersections.get(b)
    x_b = point_b[0]
    y_b = point_b[1]
...
def create_graph(maps):
    graph = [list() for _ in range(len(maps.intersections))]
    for i in range(0, len(maps.roads)):
        for point in maps.roads[i]:
            graph[i].append(
                (point, calculate_distance(maps, i, point)))
    return graph


def minimum_cost(maps, graph, start, goal):
    ...
    while len(min_heap) > 0:
        ...
        for neighbor, edge_cost in graph[current]:
            ...
            cost_edge = path[current] + edge_cost

            # (cost_edge + heuristic cost)
            cost_current = cost_edge + calculate_distance(maps, neighbor, goal)

            if neighbor not in path or cost_edge < path[neighbor]:
                path[neighbor] = cost_edge
                heapq.heappush(min_heap, (cost_current, neighbor))
